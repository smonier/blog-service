...
public class BlogMutations {
    ...
    @GraphQLField
    @GraphQLName("updateCommentStatus")
    @GraphQLDescription("Update the status of a comment (approve/reject)")
    public CommentStatusPayload updateCommentStatus(
            @GraphQLName("commentId") @GraphQLNonNull String commentId,
            @GraphQLName("status") @GraphQLNonNull String status,
            DataFetchingEnvironment environment) {
        ...
        try {
            ...
            if (success) {
                return new CommentStatusPayload(true, "Comment status updated successfully");
            } else {
                return new CommentStatusPayload(false, "Failed to update comment status");
            }
        } catch (BlogServiceException e) {
            throw new DataFetchingException("Error updating comment status: " + e.getMessage(), e);
        }
    }

    @GraphQLField
    @GraphQLName("deleteComment")
    @GraphQLDescription("Delete a comment")
    public CommentStatusPayload deleteComment(
            @GraphQLName("commentId") @GraphQLNonNull String commentId,
            DataFetchingEnvironment environment) {
        ...
        try {
            ...
            if (success) {
                return new CommentStatusPayload(true, "Comment deleted successfully");
            } else {
                return new CommentStatusPayload(false, "Failed to delete comment");
            }
        } catch (BlogServiceException e) {
            throw new DataFetchingException("Error deleting comment: " + e.getMessage(), e);
        }
    }
